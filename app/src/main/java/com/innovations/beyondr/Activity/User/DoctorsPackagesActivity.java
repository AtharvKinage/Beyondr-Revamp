...
public class DoctorsPackagesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_doctors_packages);
    ...
    private void processsearch(String s) {
        Amplify.API.query(
                ModelQuery.list(Doctor.class, Doctor.DOCTORTYPE.eq(DoctorType.DOCTOR).and(Doctor.STATUS.eq(DoctorProfileReview.PUBLISHED)).and(Doctor.NAME.beginsWith(s)).or(Doctor.GENDER.beginsWith(s)).or(Doctor.COUNTRY.beginsWith(s)).or(Doctor.SPECIALIZATION.beginsWith(s))),
                response -> {
                    Log.e("rr" , "" + response);
                    ...
                    if (String.valueOf(response.getData().getItems())  == "[]" ){
                        ...
                        for (Doctor todo : response.getData()) {
                            Amplify.API.query(
                                    ModelQuery.list(Packages.class, Packages.DOCTORTYPE.eq(DoctorType.DOCTOR).and(Packages.DOCTOR_ID.eq(todo.getUserId()))),
